...
from utils import (
    ...
    apply_deadzone_value, apply_linearity
)
...
def simulate_key_press(kname):
    ...
    for idx, ci in enumerate(cfg.get("custom_inputs", [])):
        ...
        if ci.get("bind_key") == kname:
            ...
            # Käsittele dpad_left ja dpad_right mukautetuissa kohteissa
            if target == "dpad_left":
                buttons_pressed.add("DPAD_LEFT")
            elif target == "dpad_right":
                buttons_pressed.add("DPAD_RIGHT")
            else:
...
def on_key_press_raw(kname):
    ...
    lim_key = cfg.get("left_stick_limiter", {}).get("bind_key", "").lower()
    if kname == lim_key and lim_key != "":
    ...
    l3_bind = cfg.get("keyboard", {}).get("l3", {}).get("bind_key", "").lower()
    if kname == l3_bind and l3_bind != "":
        if cfg.get("left_stick_limiter", {}).get("disable_on_run", False):
            limiter_active = False
...
def on_key_release_raw(kname):
    ...
    for idx, ci in enumerate(cfg.get("custom_inputs", [])):
        ...
        if ci.get("bind_key") == kname:
            target = ci.get("target")
            if target == "dpad_left":
                buttons_pressed.discard("DPAD_LEFT")
            elif target == "dpad_right":
                buttons_pressed.discard("DPAD_RIGHT")
            else:
                xinp = cfg.get("keyboard", {}).get(target, {}).get("xinput", "").upper()
                if xinp in ("LEFT_TRIGGER", "RIGHT_TRIGGER"):
                    if xinp in triggers_pressed: triggers_pressed.discard(xinp)
...
def on_click(x, y, keyname, pressed):
    ...
    for idx, ci in enumerate(cfg.get("custom_inputs", [])):
        ...
        if ci.get("bind_key") == keyname:
            target = ci.get("target")
            if target == "dpad_left":
                if pressed: buttons_pressed.add("DPAD_LEFT")
                else: buttons_pressed.discard("DPAD_LEFT")
            elif target == "dpad_right":
                if pressed: buttons_pressed.add("DPAD_RIGHT")
                else: buttons_pressed.discard("DPAD_RIGHT")
            else:
                xinp = cfg.get("keyboard", {}).get(target, {}).get("xinput", "").upper()
                if xinp in ("LEFT_TRIGGER", "RIGHT_TRIGGER"):
                    if pressed: triggers_pressed.add(xinp)
...
def on_scroll(x, y, dx, dy):
    ...
    for idx, ci in enumerate(cfg.get("custom_inputs", [])):
        ...
        if ci.get("bind_key") == keyname:
            target = ci.get("target")
            if target == "dpad_left":
                pulse_button("DPAD_LEFT")
            elif target == "dpad_right":
                pulse_button("DPAD_RIGHT")
            else:
                xinp = cfg.get("keyboard", {}).get(target, {}).get("xinput", "").upper()
                if xinp:
                    pulse_button(xinp)
...
def update_loop():
    ...
    global target_rx, target_ry, raw_mouse_x, raw_mouse_y
    ...
    while running:
        ...
        if target_lx == 0.0 and target_ly == 0.0:
            lx = left_stick_state["x"]
            ly = left_stick_state["y"]
        ...
        # Luentajonon tyhjennys ja laskenta luetaan aina jokaisella silmukan kierroksella
        if mouse_dx_queue:
        ...
        # Ohjaimen oikean tatin arvo: yhdistetään akselisyötteet (passthrough + hiiri)
        target_rx = max(-1.0, min(1.0, target_rx + vx))
        target_ry = max(-1.0, min(1.0, target_ry + vy))
